    ...
    public function add()
{
    ...
    if ($this->form_validation->run() == false) {
        ...
        if ($insert) {
            ...
            // Log or display the database error message
            $db_error = $this->db->error();
            error_log('Database error: ' . $db_error['message']);
            
            // Set an error message to inform about the issue
            set_pesan('Oops, ada kesalahan saat menyimpan data! Silakan coba lagi.');
